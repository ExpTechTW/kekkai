...
**Lockout 防護**：config 載入時檢查 `private.tcp` 含 22 但 `ingress_allowlist` 空 → 拒絕啟動，錯誤訊息直接指出會鎖掉 SSH。用 `kekkai check <path>` 可以離線驗證。
...
kekkai/
├── cmd/
│   ├── kekkai-agent/             # daemon binary
│   └── kekkai/                   # CLI + TUI binary
├── bpf/
│   ├── xdp_filter.c              # strict-policy XDP program
│   └── headers.h                 # self-contained eth/ip/tcp/udp (no BTF)
├── internal/
│   ├── config/
│   │   ├── config.go             # schema + validation + normalize
│   │   ├── default.yaml          # canonical template, go:embed target
│   │   ├── template.go           # Render(values) → filled YAML
│   │   ├── migrate.go            # version detect + parse
│   │   └── backup.go             # update/auto/manual backup kinds
│   ├── loader/
│   │   ├── loader.go             # platform API + Options
│   │   ├── loader_linux.go       # real XDP attach + xdp_mode fallback
│   │   ├── loader_stub.go        # non-linux dev stub
│   │   └── bpf/xdp_filter.o      # go:embed target (produced by make bpf)
│   ├── maps/
│   │   ├── blocklist.go          # PrefixSet (shared by blocklist/allowlist)
│   │   └── portset.go            # PortSet (big-endian u16 HASH)
│   ├── stats/                    # dual-goroutine stats reader
│   ├── tui/                      # Bubble Tea TUI for `kekkai status`
│   └── doctor/                   # health checks for `kekkai doctor`
├── deploy/
│   └── systemd/kekkai-agent.service
├── scripts/
│   └── build-bpf.sh              # clang → xdp_filter.o wrapper
├── kekkai.sh                     # one-script installer/updater/repair
├── Makefile
├── readme.md
└── COMMAND_ZH.md                 # full operator handbook (Chinese)
```
...
Canonical template 在 [internal/config/default.yaml](internal/config/default.yaml)（含中英雙語註解），編譯時 `go:embed` 打包進 binary。`kekkai reset` 輸出的就是這份 template 經過 `Render(values)` 字串替換後的成品，註解全數保留。
...
Config 頂層 `version: 1`。目前是初始發佈版本，schema 還沒有 breaking change，所以沒有實際的 migration 路徑 — 遇到 `version: 2+` 的檔案會直接拒絕啟動。`internal/config/migrate.go` 預留了未來第一次改 schema 時的 plug-in 點，屆時遷移邏輯會直接走 `Render(translatedValues)`，**註解自動跟著保留**（因為 template 是共用的）。
...
目前 schema 是 v1，沒有實際的 migration 發生 — `check` / `show` 純 memory 讀取。未來真正升版時，daemon 啟動流程會做「讀舊版 → Render 新 template + 舊值 → 備份舊檔 → 寫新檔」，`check` / `show` 只在 memory 中模擬不寫回。
