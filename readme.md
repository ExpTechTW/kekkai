...
```bash
git clone <repo> kekkai && cd kekkai
bash scripts/bootstrap.sh          # 裝依賴、build、install binary、寫預設 config
sudo nano /etc/kekkai/kekkai.yaml    # 填 interface.name 和 filter 規則
kekkai-agent -check /etc/kekkai/kekkai.yaml   # 先驗證
...
kekkai-agent -check /etc/kekkai/kekkai.yaml   # 建議先驗
sudo systemctl reload kekkai-agent          # 或 sudo kill -HUP $(pgrep kekkai-agent)
```
...
5. 用**新 binary** 驗證 `/etc/kekkai/kekkai.yaml`，失敗就中止安裝
6. 舊 binary 備份到 `/usr/local/bin/kekkai-agent.prev`
...
```bash
kekkai-agent -check /etc/kekkai/kekkai.yaml     # 啟動前驗證
kekkai-agent -show  /etc/kekkai/kekkai.yaml     # 看 agent 實際解讀成什麼樣
kekkai-agent -backup /etc/kekkai/kekkai.yaml    # 手動備份
```
